...
export function OrderStatus({ initialOrder }: OrderStatusProps) {
    ...
    const downloadQRCode = () => {
        const canvas = document.getElementById("order-qr") as HTMLCanvasElement
        if (!canvas) return
        const pngUrl = canvas.toDataURL("image/png").replace("image/png", "image/octet-stream")
        const downloadLink = document.createElement("a")
        downloadLink.href = pngUrl
        downloadLink.download = `order-${order.id.slice(0, 8)}.png`
    ...
    return (
        <div className="space-y-6">
            ...
            {order.payment_status === 'paid' && (
                <Card className="w-full max-w-md mx-auto border-dashed border-2 bg-muted/5">
                    <CardHeader className="text-center pb-2">
                        ...
                        <CardDescription>
                            Show this QR at the shop to verify your order
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="flex flex-col items-center gap-6 pb-6 pt-2">
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            className="w-full max-w-[200px] h-10 gap-2 font-medium"
